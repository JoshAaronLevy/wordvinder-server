...
const DEFAULT_BASE_URL = 'https://api.dify.ai/v1/chat-messages';
...
function resolveUserId(requestContext) {
  ...
  // If deployed behind a proxy/CDN, Express "trust proxy" may be needed for accurate IPs.
...
function ensureConfig() {
  const { baseUrl, apiKey, appId } = getEnvConfig();
  if (!apiKey) {
    const error = new Error('Missing DIFY_API_KEY');
    error.code = 'DIFY_ERROR';
    throw error;
  }
  return { baseUrl, apiKey, appId };
}
...
function extractModelText(responseJson) {
  ...
  if (responseJson.data) {
    if (typeof responseJson.data.answer === 'string') {
      return responseJson.data.answer;
    }
    if (typeof responseJson.data.text === 'string') {
      return responseJson.data.text;
    }
    if (responseJson.data.outputs && typeof responseJson.data.outputs.text === 'string') {
...
async function runBoardExtraction({ fileBuffer, fileName, mimeType, requestContext }) {
